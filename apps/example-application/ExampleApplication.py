...
from ImageDescription import describe_image
...
def main():  # pylint: disable=too-many-locals
    ...
    while True:
        msg = am.read_message(timeout=10)  # or timeout can be zero to poll
                ...
                if tmpfile_path:
                    ...
                    try:
                        with open(tmpfile_path, 'rb') as file:
                            # print(content)
                            description = describe_image(tmpfile_path)
                            tags = {'watsonx_description': description}
                            print(f"Assigned description: \"{description}\" to {key.path.decode(ENCODING)}")
